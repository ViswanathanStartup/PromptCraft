    ...
    <div className="space-y-8">
      <div className="text-center mb-8">
        ...
        <p className="text-slate-400 text-sm mt-2">
          Follows Sriram Srinivasan's CLEAR Framework
        </p>
      </div>
              ...
              <div className="flex gap-2">
                ...
                {analysis && analysis.overallScore >= INDUSTRY_STANDARD ? (
                  ...
                  <button
                    ...
                    className="flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700 disabled:bg-slate-600 text-white rounded-xl transition-all duration-300 font-medium text-sm shadow-lg hover:shadow-glow hover:scale-105 active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100"
                  >
                ...
                {prompt.trim() && (
                  <button
                    onClick={() => setPrompt('')}
                    className="flex items-center gap-2 px-4 py-2 bg-red-600 hover:bg-red-700 text-white rounded-xl transition-all duration-300 font-medium text-sm shadow-lg hover:shadow-xl hover:scale-105 active:scale-95"
                    title="Clear prompt"
                  >
                    <X className="w-4 h-4" />
                    Clear
                ...
                <button
                  ...
                  disabled={!prompt.trim()}
                  className="p-2 bg-slate-600 hover:bg-slate-700 disabled:bg-slate-700 text-white rounded-lg transition-all duration-200 hover:scale-110 active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100"
                  title="Copy prompt"
                >
...
💡 Tip: Use the Templates tab to browse 200+ professional prompt examples!"
              className="w-full h-96 sm:h-[32rem] bg-slate-800/50 border border-slate-600 rounded-xl sm:rounded-2xl p-3 sm:p-4 text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent resize-y transition-all duration-200 text-sm sm:text-base"
            />
            ...
            {analysis && (
              <div className="mt-4 flex flex-col sm:flex-row sm:items-center justify-between gap-2 text-xs sm:text-sm text-slate-400">
                ...
                <span className="text-slate-400 text-xs">Use save button to add to history</span>
              </div>
            )}
          </div>
          ...
          <div className="bg-gradient-to-r from-blue-500/10 to-purple-500/10 backdrop-blur-sm rounded-xl sm:rounded-2xl p-4 sm:p-6 border border-blue-500/20">
            ...
            <ul className="space-y-2 text-sm text-slate-300">
              ...
              <li>🚀 Click the Optimize button to improve your prompt quality</li>
            </ul>
          </div>
        </div>
