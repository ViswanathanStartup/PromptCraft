...
import {
  Search, Star, Users, Code, Heart, Copy, ArrowRight,
  Grid, List, BookOpen, Target,
  Eye, Check
} from 'lucide-react';
import { EmptyTemplates } from './EmptyState';
...
const TemplatesTab = ({ onLoadPrompt, onCopySuccess, onToggleFavorite, isFavorited }) => {
  ...
  const TemplateCard = ({ template, isExpanded, onToggleExpanded }) => (
    <div className="bg-white/5 backdrop-blur-sm rounded-2xl p-5 sm:p-6 border border-white/10 hover:border-purple-500/50 hover:shadow-xl hover:shadow-purple-500/10 transition-all duration-300 group">
      {/* Header Section - Most Prominent */}
      <div className="mb-4">
        <div className="flex items-start justify-between gap-3 mb-3">
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-2">
              <div className={`p-1.5 ${template.for_devs ? 'bg-purple-600/20' : 'bg-blue-600/20'} rounded-lg flex-shrink-0`}>
                {template.for_devs ? <Code className="w-4 h-4 text-purple-400" /> : <Users className="w-4 h-4 text-blue-400" />}
              </div>
              <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${getCategoryColor(template.for_devs)}`}>
                {template.for_devs ? 'Development' : 'General'}
              </span>
              <span className="text-slate-500 text-xs">#{template.id}</span>
            </div>
            <h3 className="text-white text-lg sm:text-xl font-bold mb-2 group-hover:text-purple-300 transition-colors leading-tight">
              {template.act}
            </h3>
          </div>
      ...
      <div className="flex items-center gap-2">
        <button
          ...
          className="flex-1 flex items-center justify-center gap-2 bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700 text-white px-4 py-2.5 rounded-xl transition-all duration-300 font-semibold text-sm shadow-lg hover:shadow-xl hover:scale-105"
        >
        ...
        <button
          ...
          className="p-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-xl transition-all shadow-lg hover:shadow-xl hover:scale-105"
          title="Copy prompt"
        >
        ...
        <button
          ...
          className={`p-2.5 ${isExpanded ? 'bg-purple-600' : 'bg-slate-600'} hover:bg-purple-700 text-white rounded-xl transition-all shadow-lg hover:shadow-xl hover:scale-105`}
          title={isExpanded ? "Collapse" : "View full prompt"}
        >
      ...
      {isExpanded && (
        <div className="border-t border-white/10 mt-5 pt-5">
          <div className="bg-slate-800/50 rounded-xl p-4 border border-slate-700">
            ...
            <div className="bg-slate-900/80 rounded-lg p-4 max-h-64 overflow-y-auto border border-slate-700">
          ...
          <div className="flex gap-3 mt-4">
            <button
              onClick={() => handleUseTemplate(template)}
              className="flex-1 flex items-center justify-center gap-2 bg-green-600 hover:bg-green-700 text-white px-4 py-3 rounded-xl transition-all font-semibold text-sm shadow-lg hover:shadow-xl"
            >
            ...
            <button
              onClick={() => copyToClipboard(template.prompt)}
              className="flex-1 flex items-center justify-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-4 py-3 rounded-xl transition-all font-semibold text-sm shadow-lg hover:shadow-xl"
            >
  ...
  const TemplateListItem = ({ template }) => (
    <div className="bg-white/5 backdrop-blur-sm rounded-xl p-4 border border-white/10 hover:border-purple-500/50 hover:shadow-lg hover:shadow-purple-500/10 transition-all duration-300 group">
      <div className="flex items-center justify-between gap-4">
        <div className="flex items-center gap-4 flex-1 min-w-0">
          <div className={`p-2 ${template.for_devs ? 'bg-purple-600/20' : 'bg-blue-600/20'} rounded-lg flex-shrink-0`}>
            {template.for_devs ? <Code className="w-5 h-5 text-purple-400" /> : <Users className="w-5 h-5 text-blue-400" />}
          </div>
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-2 mb-1">
              <h3 className="text-white text-base font-bold group-hover:text-purple-300 transition-colors">
                {template.act}
              </h3>
              <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${getCategoryColor(template.for_devs)}`}>
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          ...
          <button
            onClick={() => copyToClipboard(template.prompt)}
            className="p-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg transition-all shadow-lg hover:shadow-xl hover:scale-105"
            title="Copy prompt"
          >
            <Copy className="w-4 h-4" />
          </button>
          <button
            onClick={() => handleUseTemplate(template)}
            className="flex items-center gap-2 bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700 text-white px-4 py-2 rounded-lg transition-all font-semibold text-sm shadow-lg hover:shadow-xl hover:scale-105"
          >
            <Check className="w-4 h-4" />
            Use
          </button>
