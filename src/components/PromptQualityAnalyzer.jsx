...
import {
  ...
  Zap, Copy, Wand2, Coffee, RotateCcw, Trash2, Download, History, ChevronDown,
  Palette, Plus
} from 'lucide-react';

// Components
import { CopySuccessNotification } from './Notifications';
...
const aiModels = {
  'gpt-4': {
    ...
    maxWords: 300,
    style: {
      prefix: 'You are a professional expert.',
      suffix: '\n\nProvide detailed, accurate responses.',
      emphasis: 'accuracy and detail'
    }
  },
  'claude': {
    name: 'Claude',
    ...
    style: {
      ...
      emphasis: 'thoughtfulness and clarity'
    }
  },
  'gemini': {
    name: 'Gemini',
    ...
    style: {
      ...
      emphasis: 'conciseness and helpfulness'
    }
  }
};
...
function PromptCraft() {
  ...
  // Calculate analysis using useMemo to avoid unnecessary recalculations
  const analysis = useMemo(() => {
    return analyzePrompt(prompt);
  }, [prompt]);
  const [selectedAIModel] = useState('gpt-4');
  const [modelChangeNotification] = useState('');
  ...
  const quickFixes = {
    ...
    breakIntoMultiple: () => {
      ...
      const broken = `**PROMPT PART 1:**
${part1}

**Instructions for Part 1:** Please complete this first, then I'll provide Part 2.

---

**PROMPT PART 2 (Save for next message):**
${part2}

**Usage:** Copy Part 1 above for your first message, then use Part 2 in a follow-up message.`;
  ...
  const getQuickFixSuggestions = () => {
    ...
    const suggestions = [];
    const wordCount = analysis.stats.words;
    
    // Length-based suggestions (priority for very long prompts)
    if (wordCount > 400) {
      suggestions.push({ 
        key: 'breakIntoMultiple', 
        label: 'Break Into Parts', 
        action: quickFixes.breakIntoMultiple,
        priority: true,
...
Generated by Prompt Craft - https://promptcraft.app`;
  ...
  const loadFromHistory = (historyItem) => {
    setPreviousPrompt(prompt); // Store current prompt for undo
  ...
  // Get prompt length status with visual indicators
  const getPromptLengthStatus = (wordCount) => {
    if (wordCount <= 150) return { 
      status: 'optimal', 
      color: 'green', 
      bgColor: 'bg-green-500/20 text-green-400',
      message: '✅ Optimal length for clarity',
      icon: '🎯'
    };
    if (wordCount <= 300) return { 
      status: 'good', 
      color: 'yellow', 
      bgColor: 'bg-amber-500/30 text-amber-300 border-amber-400/50',
      message: '⚠️ Getting lengthy - consider condensing key points',
      icon: '📏'
    };
    if (wordCount <= 500) return { 
      status: 'warning', 
      color: 'orange', 
      bgColor: 'bg-orange-500/30 text-orange-300 border-orange-400/50',
      message: '⚠️ Long prompt - may reduce AI effectiveness',
      icon: '📋'
    };
    return { 
      status: 'too-long', 
      color: 'red', 
      bgColor: 'bg-red-500/30 text-red-300 border-red-400/50',
      message: '❌ Very long - strongly consider breaking into parts',
      icon: '📄'
    };
  };
  ...
  const getRecommendations = () => {
    ...
    const currentModel = aiModels[selectedAIModel];
    const maxWords = currentModel.maxWords;
    
    // Word recommendations based on current model's MAX_WORDS
    const optimalWords = Math.floor(maxWords * 0.7); // 70% of max
    const wordStatus = words >= maxWords ? 'exceeded' : 
    ...
    return {
      words: { 
        count: words, 
        status: wordStatus, 
        recommendation: `${optimalWords}-${maxWords} words optimal` 
      },
      chars: { 
        count: chars, 
        status: charStatus, 
...
Example: You are a senior marketing strategist. Analyze the following campaign data and provide 3 specific recommendations for improving conversion rates. Focus on actionable insights that can be implemented within 30 days."
                  ...
                  {analysis && (
                    <div className="mt-4 space-y-3">
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-3">
                          {/* Length Status Indicator */}
                          {(() => {
                            const lengthStatus = getPromptLengthStatus(analysis.stats.words);
                            return (
                              <div className={`flex items-center gap-1 px-2 py-1 rounded-full text-xs ${lengthStatus.bgColor}`}>
                                <span>{lengthStatus.icon}</span>
                                <span className="hidden sm:inline">{lengthStatus.status}</span>
                              </div>
                            );
                          })()}
                      ...
                      {/* Length Guidance Message */}
                      {(() => {
                        const lengthStatus = getPromptLengthStatus(analysis.stats.words);
                        if (lengthStatus.status === 'optimal') return null;
                        
                        return (
                          <div className={`text-xs p-3 rounded-lg ${lengthStatus.bgColor} border font-medium`}>
                            ...
                            <div className="flex flex-wrap gap-2">
                              {quickFixSuggestions.map((suggestion) => (
                                <button
                                  ...
                                  className={`flex items-center gap-1 px-3 py-1 rounded-full text-xs transition-colors border ${
                                    suggestion.priority 
                                      ? 'bg-orange-600/20 hover:bg-orange-600/30 text-orange-300 hover:text-orange-200 border-orange-500/30' 
                                      : 'bg-purple-600/20 hover:bg-purple-600/30 text-purple-300 hover:text-purple-200 border-purple-500/30'
                                  }`}
                ...
                <div className="bg-gradient-to-r from-blue-500/10 to-purple-500/10 backdrop-blur-sm rounded-xl sm:rounded-2xl p-4 sm:p-6 border border-blue-500/20">
                  ...
                  <ul className="space-y-2 text-sm text-slate-300">
                    ...
                    <li>🧠 Focus on clarity and specific instructions</li>
